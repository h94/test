...
- **actionlog（操作記錄）**：僅特定業務操作（如賽事比分拆分/合併、隊伍校正等）時，由本服務 INSERT；`date`（分區鍵，格式 yyyyMMdd）與 `addtime`（操作時間戳）務必準確，查詢時以此排序。寫入時需正確設定 `actionclass`（操作類別，如 SiteTeam、Auto）及 `action`（操作名稱，如 Split）。

### 讀取規則

- **帳號查詢（登入/驗證）**：須篩選 `enabled = 1`，已關閉或凍結帳號不可登入。依帳號 `account`（主鍵）精確查詢。
- **帳號關停查詢**：查詢已關閉帳號時，應使用 `closetime IS NOT NULL` 且 `enabled = 0`。
- **跨站點帳號查詢**：`accounts_*` 系列表（accounts_AU8、accounts_Fortuna888、accounts_HGA、accounts_HGA2、accounts_KKK、accounts_KU、accounts_NK、accounts_Panda、accounts_TG、accounts_TG999）對應不同站點，查詢時需根據站點代碼選擇正確的表名。
- **操作記錄查詢**：必須以 `date`（分區鍵）為條件，不可跨大量分區查詢；再依 `addtime`、`user`、`gametype` 等集群鍵排序。常見 `actionclass` 值包括 `SiteTeam`（站點隊伍操作）、`Auto`（自動操作）等。
...
- **gameusers_wallet_transactions**：僅錢包服務可 INSERT，pricecenterservice 僅讀取（用於報表統計）。
- **notification_sitemails**：僅站內信服務（notificationService）可寫入；pricecenterservice 僅讀取（如提供站內信列表 API）。
- **chatroomhistories** / **chatroomhistories_backup**：僅聊天服務（chatService）可寫入；pricecenterservice 僅讀取聊天記錄（如用於聊天歷史查詢）。
- **community_groups**：僅社群管理服務（communityService）可寫入；pricecenterservice 僅讀取群組資訊。
- **bk_siteplayers**：僅後台管理介面或資料同步作業可寫入；不得由一般 API 直接變更。
- **leagues_{gameType}、games_{gameType}、teams_{gameType}、date_leagues、automapteams、automaperrs、alertlog、actionlog、datum_log、inplaysrepadlogs、memberdailyreport、notification_messages、notification_topics、predictdailyeport**：以上皆為唯讀資料，由數據同步、後台管理或其他服務維護，pricecenterservice 不得執行 INSERT / UPDATE / DELETE。
...
- **站內信查詢**：須以 `Account` 精確對應收件者，不可跨帳號讀取。
- **社群群組列表**：若 API 需要啟用群組，應過濾 `Enabled = 1`，禁用群組不應顯示。
- **球員資料 (bk_siteplayers)**：查詢時通常以 `Site + SiteID + Year` 複合條件獲取單一球員資料；無需特殊業務過濾。
- **賽事相關查詢**：
  - leagues_{gameType}：按聯賽 ID 或 Continent 等條件過濾。
  - games_{gameType}：必須以 `gdate`（比賽日期）和 `lid`（聯賽 ID）為核心過濾條件，避免全表掃描；查詢進行中賽事可搭配 `status` 過濾。
  - teams_{gameType}：按隊伍 ID 或所屬聯賽 `lid` 過濾。
  - date_leagues：依 `gdate` + `gametype` 獲取當日活躍聯賽清單。
  - automapteams / automaperrs：按 `gametype, site, sitelid` 等主鍵條件精確查詢。
  - alertlog：按 `site, gtype` 過濾，並可限制時間範圍（`addtime`）。
  - actionlog：按 `date`（分區鍵）過濾，再依 `addtime` 等排序。
  - datum_log：以 `gdate, gametype, league, gid` 等組合條件查詢，避免全分區掃描。
  - inplaysrepadlogs：以 `gdate, gtype, gid` 過濾特定比賽的滾球讓分記錄。
  - memberdailyreport：以 `Reportdate` 單鍵查詢即可。
  - notification_messages：查詢啟用訊息應過濾 `Enabled = 1`。
  - notification_topics：查詢啟用主題應過濾 `Enabled = 1`。
  - predictdailyeport：以 `Reportdate` 查詢，可搭配 `Gametype` 篩選。

### 不可回傳欄位

- **gameusers_wallet.Balance**：錢包餘額屬敏感財務資料，一般對外查詢 API 不應回傳（僅限有權限的管理端可回傳）。
- **gameusers_wallet.AuthKey**：認證金鑰，等同使用者憑證，任何 API 皆不可回傳。
- **chatroomhistories / chatroomhistories_backup.Account**：發送者帳號（若須顯示名稱應使用 UserName 或去識別化處理）。
- **notification_sitemails.Content**：站內信內容屬個人隱私，僅限收件者本人可讀取；列表 API 不應回傳全文。
- **community_groups.Owner**：群主帳號，一般列表查詢不應暴露。
- **games_{gameType}.siteidmaps**：內部站點比賽 ID 映射，不應直接暴露。
- **automapteams.ratio**、**automaperrs.errtype** 等技術細節若無業務需求，不建議直接暴露。
...
| Games DB 賽事數據維護 | collector（數據同步） | games_bk/games_bm/games_bs/games_ck 四表的數據落地與更新由 collector 處理，pricecenterservice 僅能讀取 |
...
- ❌ 未經授權讀取 `chatroomhistories` 中其他使用者的聊天內容 → ✅ 查詢時必須綁定使用者所屬群組（GID）或帳戶，並配合 chatService 權限驗證。
- ❌ 站內信列表 API 回傳完整 `Content` 欄位 → ✅ 列表應僅回傳主旨（Subject）與發送時間，全文內容應另提供單封查詢 API（且需驗證收件者身份）。
- ❌ 社群群組查詢未過濾 `Enabled = 1`，導致前端顯示已停用群組 → ✅ 預設查詢應加上啟用條件，若後台管理需要則由特定 API 提供。
- ❌ 查詢 games_{gameType} 時未限制 `gdate` 範圍 → ✅ 必須帶入日期條件，避免掃描過多分區，影響效能。
- ❌ 直接回傳 games 表中的 `siteidmaps` 給前端 → ✅ 應僅傳遞業務所需的標準資料，內部映射不應暴露。
- ❌ 在 sport 唯讀表中嘗試 INSERT 或 UPDATE → ✅ 除少數明確授權的寫入表外，一律以只讀方式操作，避免資料錯亂。
- ❌ 對 Games DB 的 games_* 表執行寫入操作 → ✅ 這些表由 collector 維護，pricecenterservice 僅可讀取，誤寫入會導致數據不一致。
