...
| 資料來源 | 角色 | 定義 |
|---------|------|------|
| Cassandra games | reader | Schema：[db/games.json](../../db/games.json) · 語意：[db/games-detail.md](../../db/games-detail.md) |

> 包含依運動類型分表的比賽基礎資料，如 `games_bk`、`games_bm`、`games_bs`、`games_ck` 等。

### 寫入限制

- **本服務不寫入** games 相關表，所有寫入（新增比賽、更新比分／狀態）由資料爬取或派彩服務負責

### 讀取規則

- **待預測的比賽**：`status='PreGame'` 且 `(gdate + gtime) > 當前時間` — 僅選取尚未開始的比賽進行預測，避免對已開賽或已結束的比賽無效操作
- **歷史賽果分析**：`status='Final'` 且 `gdate <= 今天` — 用於模型訓練、回測或策略評估
- **聯賽過濾**：可依 `lid` 篩選特定聯賽
- **來源站台過濾**：可依 `source` 篩選特定爬蟲來源的比賽，用於跨站比對或賠率分析
- **索引優化**：查詢時應盡量搭配 `gdate` 範圍、`status` 等條件，避免全表掃描

### 不可回傳欄位

- **siteidmaps**：包含外部站台的比賽 ID 映射，屬於商業機密，對外 API 不應暴露
- **create_at**：內部記錄時間戳，無業務意義，無需對外回傳
...
| 比賽基礎資料的寫入與更新 | data-collector (推測) | games 表的比賽資訊由爬蟲或資料匯入服務維護，predictrobot 僅讀取 |
...
- ❌ **誤用 predictbets_BK/BS/SC/HL/FL/ES/TN 表**：不同賽事類型對應不同表，查詢時需明確指定  
  ✅ 依 game_type 動態選擇對應的 predictbets_* 表，避免資料混亂
...
- ❌ **對 games 表查詢未過濾比賽狀態**：直接讀取所有比賽用於預測，可能對已開始或結束的比賽發起預測  
  ✅ 必須檢查 `status='PreGame'` 且 `(gdate + gtime) > 當前時間`，確保僅對未來比賽進行預測

- ❌ **全表掃描 games 表**：未加任何索引條件查詢大量歷史資料，造成效能瓶頸  
  ✅ 加入必要的過濾條件，如聯賽 ID (`lid`)、日期範圍 (`gdate`)、狀態 (`status`)，並利用 Cassandra 分區鍵提升查詢效率