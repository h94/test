...
| 資料來源 | 角色 | 定義 |
|---------|------|------|
| Cassandra pricecenter keyspace | reader | Schema：[db/pricecenter.json](../../db/pricecenter.json) · 語意：[db/pricecenter-detail.md](../../db/pricecenter-detail.md) |
...
- **比賽資料查詢**：`games_${gtype}` 動態表（如 `games_BK`、`games_FB` 等）以 `gdate` 為分區鍵，查詢時必須指定 `WHERE gdate=?`，可搭配 `lid`（聯賽 ID）與 `status`（例如 `status=2` 賽前）等 clustering key 過濾，不可跨日期掃描。
- **聯賽與球隊名稱查詢**：`leagues_${gtype}` 與 `teams_${gtype}` 以 `id` 為主鍵，通常全表掃描量極小，但應用端仍應避免使用 `ALLOW FILTERING`；建議直接依 `id` 列表查詢。
- **站點比賽對照查詢**：`sitegames_${gtype}` 提供外部站點比賽與內部比賽的對應，常見查詢為 `WHERE gid=?`（內部比賽 ID）來獲取外部 sitegid 與站點名稱，請確認實際分區鍵正確設定；若需反查（`WHERE sitegid=?`），請確認該欄位是否為分區鍵，否則應使用索引或變更查詢策略。
...
| 資料來源 | 角色 | 定義 |
|---------|------|------|
| Cassandra news keyspace | owner | Schema：[db/news.json](../../db/news.json) · 語意：[db/news-detail.md](../../db/news-detail.md) |

### 寫入限制

- **llmhashkey**：`ainews` / `ainews_gs` 中的 `llmhashkey` 由系統根據 `llmsettings` 自動計算哈希後寫入，不得手動指定或修改。
...
- **aireports.results**：由 `IAIPredictService.CalcuteAIPredicts` 計算後寫入，不可手動改寫；`bets`、`others` 同樣由系統計算寫入，外部唯讀。
- **inplaylogs**：即時比分日誌，由比賽數據服務寫入，本服務僅讀取，不進行任何寫入。
...
- **AI 預測報告讀取**：`aireports` 使用 `WHERE gdate=? AND gtype=? AND lid=?`，必須包含分區鍵 `gdate`，不可跨日期掃描；`others` 欄位包含比賽統計，讀取後由應用層解析而非依賴資料庫過濾。
- **即時日誌查詢**：`inplaylogs` 以 `gdate` 為分區鍵，查詢即時比分時使用 `WHERE gdate=? AND gtype=? AND lid=? AND gid=?`，並可根據時間戳記範圍（如 `addtime`）獲取片段，嚴禁無分區鍵掃描。
...
- **inplaylogs.logdata**：即時日誌詳細原始資料，可能包含內部格式與未發布比分，不對用戶端暴露。
...
| 資料來源 | 角色 | 定義 |
|---------|------|------|
| Cassandra predict keyspace | writer / reader | Schema：[db/predict.json](../../db/predict.json) · 語意：[db/predict-detail.md](../../db/predict-detail.md) |
...
- **可下注獎池比賽**：`betpool_games` 使用 `WHERE status=2 AND payout=false` 過濾賽前且未支付的場次；可搭配 `hot`、`viponly` 過濾，但請注意該表主鍵為 `id`，僅能以 `id` 為分區鍵精準查詢，若需要全局過濾需避免使用 `ALLOW FILTERING`，建議透過應用層快取或使用其他索引。
...
- **預測記錄查詢**：`predictbets_{gtype}` 表動態，查詢時需指定 `lid` 與 `gdate`（分區鍵），常見查詢為 `WHERE lid=? AND gdate=? AND gid=? AND account=?` 或依 `account` 過濾，須根據實際分區鍵進行，避免全表掃描。
- **預測過濾報告查詢**：`predictfilterreports` 使用 `WHERE gametype=? AND lid=? AND reportdate=?` 查詢特定遊戲類型的報告，不可無分區鍵掃描。
...
- **gameuserviews**：瀏覽記錄由系統根據使用者瀏覽行為自動寫入，`datetime`、`account`、`gtype` 由系統設定，不可手動修改或刪除。
...
- **用戶瀏覽記錄查詢**：`gameuserviews` 使用 `WHERE year=? AND account=?` 或搭配 `datetime>=?` 範圍查詢，必須指定分區鍵 `year`；跨年查詢須拆分多次或由應用層合併結果，禁止無分區鍵掃描。
...
- **gameusers.memberships**：會員資格列表視為內部資料，不對外暴露所有會員身份（但可公開基本會員層級，注意不要洩露權限細節）。
...
| 即時比分擷取 | datasource-service | `inplaylogs` 原始數據由資料源服務寫入，本服務僅讀取 |
...
- ❌ 對 `gameuserviews` 進行跨年查詢時使用 `WHERE datetime>=? AND datetime<=?` 而無 `year` 條件 → ✅ 必須指定 `year` 分區鍵，跨年需依年份拆分查詢