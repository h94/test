...
| 事項 | 負責服務 | 說明 |
|------|---------|------|
| 獎池遊戲（`betpool_games`）的建立、配置與狀態轉換 | `promotionservice` / `gamemanagementservice` | 本服務僅讀取遊戲狀態進行流速控制判斷。 |
| 投注的派彩與結算（`payout`、`profitzcoin`） | `walletservice` / `payoutservice` | 本服務不處理資金變動，最多觸發結算信號。 |
| 活動規則、周期定義（`activities_cycles`） | `activityservice` | 活動資料的維護不屬於流速控制範圍。 |
| 殺手帳戶統計指標的原始聚合計算 | `analyticsservice`（若有） | 本服務僅負責最終匯總寫入 `killeraccounts_BK`，不負責從原始投注計算 `avgodd`。 |

---

## 常見錯誤

- ❌ 在未檢查 `betpool_games.status = 1` 且 `endtime` 未過期的情況下允許用戶投注  
  ✅ 必須在流速控制前的業務邏輯中驗證遊戲狀態，並同步維護 Redis 可投注集合。

- ❌ 試圖直接 UPDATE `betpool_games.winresult` 或 `payout` 欄位  
  ✅ 這些欄位由派獎或遊戲管理服務負責；本服務僅讀取並記錄相關事件。

- ❌ 在無分散式鎖保護下對 `killeraccounts_BK` 執行 DELETE + INSERT，導致並發覆蓋  
  ✅ 必須使用 `SETNX` 鎖，確保同一 `lid`+`cid` 同時只有一個計算執行個體。

- ❌ 對外 API 在 `payout = false` 時直接回傳 `winresult`  
  ✅ 應在序列化前依 `payout` 狀態過濾該欄位。

- ❌ 使用無 `gid` 條件的查詢掃描 `betpool_bets`  
  ✅ 所有投注相關查詢必須提供 `gid`，避免跨分割區掃描。

- ❌ 在 `calculatelog` 中將 `done` 從 `1` 錯誤重設為 `0`  
  ✅ 已完成計算不得回退；若需重算應由排程新建一組 `weekid` 記錄。

- ❌ 直接修改 `activities_winneraccounts` 的中間排名數據 
...(truncated)