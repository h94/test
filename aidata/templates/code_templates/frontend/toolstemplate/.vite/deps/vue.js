...
function historyEffectScope(effect2, scope = activeEffectScope) {
...
function effect(fn, options) {
  ...
  if (options) {
    ...
    if (options.scope)
      historyEffectScope(_effect, options.scope);
  }
...
if (true) {
  getGlobalThis().__VUE_HMR_RUNTIME__ = {
    createHistory: tryWrap(createHistory),
...
function registerHMR(instance) {
  ...
  let history = map.get(id);
  if (!history) {
    createHistory(id, instance.type);
    history = map.get(id);
  }
  history.instances.add(instance);
}
function unregisterHMR(instance) {
  map.get(instance.type.__hmrId).instances.delete(instance);
}
function createHistory(id, initialDef) {
  if (map.has(id)) {
    return false;
  }
  map.set(id, {
...
function rerender(id, newRender) {
  ...
  history.initialDef.render = newRender;
  [...history.instances].forEach((instance) => {
    if (newRender) {
...
function reload(id, newComp) {
  ...
  updateComponentDef(history.initialDef, newComp);
  const instances = [...history.instances];
  for (const instance of instances) {
    const oldComp = normalizeClassComponent(instance.type);
    if (!hmrDirtyComponents.has(oldComp)) {
      if (oldComp !== history.initialDef) {
...
var TransitionGroupImpl = {
  ...
  setup(props, { slots }) {
    ...
    onUpdated(() => {
      ...
      prevChildren.forEach(historyPosition);
