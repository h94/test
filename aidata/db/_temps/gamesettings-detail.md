...
| gamesettingsite | SELECT WHERE businesscode = ? | 前台查詢業務資訊 | 必須以 businesscode 精確查詢，禁止全表掃描 |
| pricebackendservice | SELECT WHERE businesscode = ? | 驗證訂閱有效性 | 同上 |

**⚠️ 跨服務限制**：
- 任何服務均不得對 `businesscode` 執行 UPDATE；若需變更業務識別碼，應建立新紀錄，不可直接修改。
- **所有查詢 `businesses` 的服務都必須以 `businesscode` 作為完整條件**，不得執行無條件的掃描。
...
| gamesettingsite | SELECT WHERE businesscode = ? AND account = ? | 登入驗證 | 必須以完整主鍵查詢 |
...
| gamesettingsite | SELECT | 驗證權限 | 讀取角色以決定可操作範圍 |
...
| 服務 | 操作 | 條件／時機 | 說明 |
|------|------|-----------|------|
| gamesettingservice | INSERT | 新增遊戲類型設定 | 必須提供完整主鍵，建立後不可 UPDATE 主鍵 |
| syncservice | INSERT | 同步設定 | 同上 |
| gamesettingsite | SELECT WHERE company = ? AND gametype = ? | 查詢前台遊戲設定 | **必須帶入 company + gametype 完整主鍵**，禁止全表掃描 |
| pricebackendservice | SELECT WHERE company = ? AND gametype = ? | 賠率計算時讀取配置 | 同上 |
...
| gamesettingsite | SELECT | 前台展示設定 | 讀取後可重新組裝，不應直接暴露原始字串 |
...
| gamesettingsite | SELECT | 前台讀取 | 用於判斷過濾條件 |
...
| gamesettingsite | SELECT | 前台讀取 | 決定顯示模式 |
...
以下表在服務摘要（gamesettingsite）或現有文件中被引用，但未出現在本次 dbSchema（`gamesettings.json`）中，請確認其存在與結構。

- **game_settings**：現有文件與服務摘要提及此表（如 `game_settings.id`、`game_settings.settings`），Redis 快取鍵 `gamesettings:game:{id}` 與之對應。若該表已遷移或廢棄，請移除相關章節；若仍存在，請補充完整 Table 定義並移除本衝突提示。
- **league_logs**：服務摘要 `gamesettingsite` 寫入限制中提到 `league_logs.company + gametype` 複合主鍵不可更新，但 schema 中未包含。需要確認此表是否歸屬於本 keyspace 或其他資料庫。
...
- ❌ `gametype_settings` 變更後未清除 Redis 快取 `gamesettings:company:{company}:gametypes` → 前台可能讀取到過期配置。
- ❌ 查詢 `businesses` 或 `gametype_settings` 時未使用完整主鍵（如全表掃描） → 效能問題與違反 Cassandra 查詢規範，gamesettingsite 等服務應強制帶入必要條件。
- ❌ 手動填寫 `updatetime` 或 `updater` → 應由服務端自動產生，避免時間不一致與偽造。