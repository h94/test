...
| gamesettingsite | ⚠️ 待人工（摘要聲稱 owner，此處定為 reader） | 唯讀（展示用途） |
| zaiservice | ⚠️ 待人工（摘要聲稱 owner，此處定為 reader） | 唯讀（內部查詢） |
| pricebackendservice | reader | 唯讀（組合展示資料） |
| pricecentersite | reader | 唯讀（夜間排程讀取，用於組合價格相關資訊） |

⚠️ **衝突待人工**：  
1. `gamesettingsite` 服務摘要聲明對 `news` keyspace 擁有 **owner / writer / reader** 權限，但此處暫定為 `reader`；多處欄位操作明細中標記衝突。  
2. `zaiservice` 服務摘要聲明對 `news` keyspace 擁有 **owner** 權限，但此處暫定為 `reader`，跨服務限制亦僅允許 SELECT。
...
⚠️ **衝突待人工**：`zaiservice` 服務摘要將 `status` 定義為 `2`=賽前, `0`=實況, `1`=賽後, `>2`=生成中，且樣本出現 `status=11`，可能為不同業務場景共用欄位，需統一。
...
| zaiservice | SELECT WHERE status=1 | 內部查詢 | 唯讀 |
| pricebackendservice | SELECT WHERE status IN (1,2) | 組合價格資訊 | 唯讀，取得已回應的新聞 |
| pricecentersite | SELECT WHERE status IN (1,2) | 夜間排程讀取 | 唯讀，組合價格相關資訊 |
...
- pricebackendservice、pricecentersite 僅有 SELECT 權限，不可執行任何寫入操作
...
| gamesettingsite | — | — | ⚠️ 衝突待人工：若 gamesettingsite 有寫入權限，不可手動修改 |
...
- ❌ 外部服務直接寫入 llmsettings → 僅 newsservice 或 gamesettingsite 管理後台可寫入（⚠️ 衝突待人工）
